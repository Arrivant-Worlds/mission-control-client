...
import styles from './leader_block_styles';
import { LeaderboardResponse, MainProps } from 'interfaces';

interface LeaderBlockProps { 
  rank: number
  user_data: MainProps['user_data']
  item_data: LeaderboardResponse['userStats'][0]
}

export const LEADER_BLOCK = (props: LeaderBlockProps) => {
  const laurel_render = () => {
    if (props.rank === 0) {
      return (
